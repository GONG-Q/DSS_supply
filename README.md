...
│ └── Optimized adversarial prompts generated by P4D for different target
│ concepts.
│
├── p4d_*_prompt_report.json
│ └── Optimization records and attack statistics corresponding to P4D prompts.
│
├── *_optimized_adversarial_prompts.txt
│ └── DSS-aware adaptive adversarial prompts optimized against the DSS
│ pipeline for different concepts.
│
└── *_optimized_adversarial_prompts_report.txt
└── Corresponding optimization logs and attack information.
...
robustness experiments introduced during the minor revision. The main artifact
execution workflow remains unchanged and is described below.
